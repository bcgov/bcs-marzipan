import { pgTable, serial, uuid, integer, boolean, varchar, timestamp, primaryKey, } from 'drizzle-orm/pg-core';
...
export const ministries = pgTable('ministries', {
    ...
    secondContactUserId: integer('second_contact_user_id').references(() => users.id), // FK to User
 ...
 */
export const podMinistries = pgTable('pod_ministries', {
 ...
 */
export const podSharedWithTeams = pgTable('pod_shared_with_teams', {
    podId: integer('pod_id')
        .notNull()
        .references(() => pods.id),
...
export const podSharedWithTeamsRelations = relations(podSharedWithTeams, ({ one }) => ({
    pod: one(pods, {
