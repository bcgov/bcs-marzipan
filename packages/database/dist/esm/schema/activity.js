import { pgTable, serial, timestamp, date, time, varchar, text, integer, boolean, uuid, bigint, check, } from 'drizzle-orm/pg-core';
import { relations, sql } from 'drizzle-orm';
import { activityStatuses, dateStatuses, timeStatuses, eventPlanners, newsReleaseDistributions, premierRequested, newsReleaseOrigins, } from './lookups';
import { organizations } from './organizations';
import { users } from './user';
import { activityThemes, activityTags, activitySubscriptions, activityCategories, activityCommsMaterials, activityTranslationsRequired, activityRepresentatives, activitySharedWithTeams, activityCommsContacts, activitySectors, favoriteActivities, activityReportSettings, } from './relations';
...
export const activities = pgTable('activities', {
    ...
    // News Release
    newsReleaseOriginId: integer('news_release_origin_id').references(() => newsReleaseOrigins.id), // FK to NewsReleaseOrigin lookup table
    ...
    // Event
    eventPlannerLeadId: integer('event_planner_lead_id').references(() => eventPlanners.id), // FK to EventPlanner (mutually exclusive with eventPlannerLeadName)
    ...
    newsReleaseDistributionId: integer('news_release_distribution_id').references(() => newsReleaseDistributions.id), // FK to NewsReleaseDistribution - maps to legacy NRDistributionId
    premierRequestedId: integer('premier_requested_id').references(() => premierRequested.id), // FK to PremierRequested - maps to legacy PremierRequestedId
    ...
    check('lead_org_xor', sql `(${table.leadOrgId} IS NULL) <> (${table.leadOrgName} IS NULL)`),
    // CHECK constraint: exactly one of eventPlannerLeadId or eventPlannerLeadName must be provided (XOR)
    check('event_planner_lead_xor', sql `(${table.eventPlannerLeadId} IS NULL) <> (${table.eventPlannerLeadName} IS NULL)`),
]);
// Relations
