...
class SeedRunner {
     ...
     */
    async recordSeedApplied(filename) {
        await this.db.execute((0, drizzle_orm_1.sql) `INSERT INTO _seed_history (filename) VALUES (${filename}) ON CONFLICT (filename) DO NOTHING`);
    }
    /**
    ...
    discoverSeedFiles() {
        ...
        const seedFiles = files
            ...
            return (file.endsWith('.sql') && /^\d{4}_/.test(file) && /seed/i.test(file));
        })
     ...
     */
    async executeSeedFile(filename, options = {}) {
        ...
        try {
            ...
            for (const statement of statements) {
                await this.db.execute(drizzle_orm_1.sql.raw(statement));
            }
