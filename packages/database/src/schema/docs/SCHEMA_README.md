...
- **Activity status (id, name) mapping**: Canonical mapping is in `packages/database/seeds/0001_20260126_lookups_seed_data.sql` (activity_statuses). Ids 1-7 must match: 1=new, 2=reviewed, 3=changed, 4=deleted, 5=delete_requested, 6=completed, 7=on_hold. The seed asserts this; code and the activities seed depend on it.
