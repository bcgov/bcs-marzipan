...
6. [Venue Presets and Last-Used Addresses](#venue-presets-and-last-used-addresses)
...
## Venue Presets and Last-Used Addresses

The activity form (Create/Edit) shows venue presets in the Venue Name combobox and as quick-select badges so users can one-click fill the venue.

### Data sources

- **Venue presets**: Stored in `venue_presets` with the same address columns as `venue_addresses` (including optional `address_line2`). Admins configure these in Administration (Venue Presets). All active rows (`is_active = true`) appear in the combobox dropdown. Rows with `is_pinned = true` are additionally displayed as badge tags beneath the input, ordered by `pinned_sort_order`. Duplicate addresses (`address_line1` + `address_line2`) are rejected on create/update.
...
- The form shows pinned preset badges plus last-used badges (total badge slots capped at 4: if 3 pinned presets are configured, only 1 last-used is shown).
- Clicking a badge sets the form’s `venueAddress` (venueName, addressLine1, addressLine2, city, provinceOrState, country) and updates the address input via the controlled `value` prop on the address autocomplete.
