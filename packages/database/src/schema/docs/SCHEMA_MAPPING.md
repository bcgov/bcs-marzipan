...
| `Significance`                      | `nvarchar(500)`     | `significance`                          | `text`                     | nullable                                                | Type changed from varchar(500) to text; optional on create/update (API + Zod)                                                                                                                                              |
...
| `EventPlannerId`                    | `int`               | _Moved to activity_event_planners_      | -                          | -                                                       | Moved to junction table activity_event_planners. Each row has optional eventPlannerId (FK to event_planners) or eventPlannerName (free text), and isLead. Multiple planners per activity; exactly one lead when non-empty. |
...
| -------------------------------- | ------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `ActivityCategories`             | `activityCategories`           | Direct mapping. Create/update API (`categoryIds`) requires at least one category when provided; service enforces on create.                                                                                                         |
...
| `FavoriteActivity`               | `userActivityFavourites`       | Replaced legacy `favorite_activities` junction table                                                                                                                                                                                |
| -                                | `activityTranslationsRequired` | New - replaces legacy Translations field                                                                                                                                                                                            |
| -                                | `activityEventPlanners`        | New - replaces single event planner on activities. Allows multiple planners per activity; each row has optional eventPlannerId (lookup) or eventPlannerName (free text), and isLead (exactly one per activity when planners exist). |
...
- `Significance` → `significance` (text, nullable)
...
- `lead_org_at_most_one`: At most one of `leadOrgId` or `leadOrgName` (both null allowed). Event planners moved to `activity_event_planners` junction table (no constraint on activities).
...
| `abbreviation`        | `varchar(6)`               | `notNull`                  | Short stable code; used in activity `displayId` when the team has no lead ministry (see `calendar-service` activity displayId generation) |
