...
| `pitchRequired`       | `bit`           | `pitchNotRequired`    | `boolean`                  | `notNull`, `default(false)`    | **Renamed and inverted** - `pitchRequired=true` in legacy maps to `pitchNotRequired=false` in new schema. Categories where pitch is NOT required have `pitchNotRequired=true`. |
