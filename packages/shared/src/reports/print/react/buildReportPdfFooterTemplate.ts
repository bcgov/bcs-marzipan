 ...
 *
 * Optional `Changed` hint (top), then a divider with 4px inset spacing, then
 * “Last updated …” (left) and page x of y (right).
...
export function buildReportPdfFooterTemplateHtml(
  ...
  const hintAndDivider = includeChangedHint
    ? `<div style="font-weight:400;font-size:11.5px;line-height:1.35;color:#64748b;">* <strong style="font-weight:700;color:#64748b;">Changed</strong> ${explanation}</div><div style="height:0;margin:4px 4px;border-top:1px solid #c5cbd3;" aria-hidden="true"></div>`
    : '';

  const mainRow = `<div style="display:flex;flex-wrap:nowrap;justify-content:space-between;align-items:baseline;gap:16px;width:100%;font-size:12px;line-height:1.45;color:#64748b;">
  ...
  return `<div style="box-sizing:border-box;width:100%;margin:0;padding:8px 24px 10px;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,'Helvetica Neue',Arial,sans-serif;background:#fff;">${hintAndDivider}${mainRow}</div>`;
}
