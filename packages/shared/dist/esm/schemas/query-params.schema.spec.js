...
import { lookupQueryParamsSchema, filterActivitiesQuerySchema, } from './query-params.schema';
describe('lookupQueryParamsSchema', () => {
    ...
    it('rejects invalid organizationId (non-UUID)', () => {
        expect(() => lookupQueryParamsSchema.parse({ organizationId: 'not-a-uuid' })).toThrow();
    });
...
describe('filterActivitiesQuerySchema', () => {
    ...
    it('transforms isIssue "true" to true', () => {
        expect(filterActivitiesQuerySchema.parse({ isIssue: 'true' }).isIssue).toBe(true);
    });
    it('transforms isIssue "false" to false', () => {
        expect(filterActivitiesQuerySchema.parse({ isIssue: 'false' }).isIssue).toBe(false);
    });
    it('rejects invalid date for startDateFrom', () => {
        expect(() => filterActivitiesQuerySchema.parse({ startDateFrom: '2025-13-45' })).toThrow();
    });
    it('rejects invalid UUID for leadMinistryId', () => {
        expect(() => filterActivitiesQuerySchema.parse({ leadMinistryId: 'not-a-uuid' })).toThrow();
    });
    it('rejects non-integer activityStatusId', () => {
        expect(() => filterActivitiesQuerySchema.parse({ activityStatusId: 'x' })).toThrow();
    });
