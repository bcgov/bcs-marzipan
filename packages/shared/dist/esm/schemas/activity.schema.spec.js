...
import { DEFAULT_LOOK_AHEAD_SECTION, DEFAULT_LOOK_AHEAD_STATUS, VISIBILITY, } from '../constants/constants';
import { createActivityRequestSchema, updateActivityRequestSchema, venueAddressFieldsSchema, } from './activity.schema';
...
describe('createActivityRequestSchema', () => {
    ...
    it('enforces title min 1 and max 255', () => {
        ...
        expect(() => createActivityRequestSchema.parse(minimalCreateRequest({ title: '' }))).toThrow();
        expect(() => createActivityRequestSchema.parse(minimalCreateRequest({ title: 'a'.repeat(256) }))).toThrow();
    });
    it('enforces summary max 1000 and significance max 1000', () => {
        expect(() => createActivityRequestSchema.parse(minimalCreateRequest({ summary: 'a'.repeat(1001) }))).toThrow();
        expect(() => createActivityRequestSchema.parse(minimalCreateRequest({ significance: 'a'.repeat(1001) }))).toThrow();
    });
    it('accepts valid enums for visibility, lookAheadStatus, lookAheadSection', () => {
        for (const v of VISIBILITY) {
            createActivityRequestSchema.parse(minimalCreateRequest({ visibility: v }));
        }
    ...
    it('rejects invalid visibility', () => {
        expect(() => createActivityRequestSchema.parse(minimalCreateRequest({ visibility: 'invalid' }))).toThrow();
    });
    it('accepts venueAddress as nullable/optional', () => {
        createActivityRequestSchema.parse(minimalCreateRequest({ venueAddress: null }));
        createActivityRequestSchema.parse(minimalCreateRequest());
        createActivityRequestSchema.parse(minimalCreateRequest({
            venueAddress: {
                venueName: 'Hall',
    ...
    it('transforms empty string to null for leadOrgId (emptyStringToNull)', () => {
        const result = createActivityRequestSchema.parse(minimalCreateRequest({ leadOrgId: '' }));
        expect(result.leadOrgId).toBeNull();
    });
    it('validates leadMinistryId as UUID', () => {
        expect(() => createActivityRequestSchema.parse(minimalCreateRequest({ leadMinistryId: 'not-a-uuid' }))).toThrow();
    });
});
