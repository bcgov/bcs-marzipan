...
import { z } from 'zod';
import { createResponseWrapperSchema, createArrayResponseWrapperSchema, } from './response-wrapper.schema';
describe('createResponseWrapperSchema', () => {
    ...
    it('rejects success: false', () => {
        expect(() => wrapperSchema.parse({ success: false, data: { id: 1, name: 'Test' } })).toThrow();
    });
    it('rejects missing data', () => {
        expect(() => wrapperSchema.parse({ success: true })).toThrow();
    });
    it('rejects wrong data shape', () => {
        expect(() => wrapperSchema.parse({ success: true, data: { id: 'x', name: 123 } })).toThrow();
    });
    it('rejects wrong data shape (missing required field)', () => {
        expect(() => wrapperSchema.parse({ success: true, data: { id: 1 } })).toThrow(); // name is required
    });
});
describe('createArrayResponseWrapperSchema', () => {
    ...
    it('rejects success: false', () => {
        expect(() => arrayWrapperSchema.parse({ success: false, data: [] })).toThrow();
    });
    it('rejects missing data', () => {
        expect(() => arrayWrapperSchema.parse({ success: true })).toThrow();
    });
    it('rejects wrong item shape in array', () => {
        expect(() => arrayWrapperSchema.parse({ success: true, data: [{ id: 'x' }] })).toThrow();
    });
});
