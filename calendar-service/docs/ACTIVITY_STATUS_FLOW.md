...
| **Delete requested** | Set when a Comms contact requests deletion (notes required).                                                 |
...
- **Request delete**: Only users who are a **Comms contact** on the activity (any entry in the activity's comms contacts, lead or not). Other editors cannot request delete.
- **Restore**: Any **Comms contact** on the activity, or **Admin**, or **System Admin**.
...
- **Request delete**: Sets status to **Delete requested**. Notes are required. Only Comms contacts on the activity may call this.
...
When status is **Delete requested** or **Deleted**, edits to the activity are not allowed. The UI shows a banner and a **Restore** button for users who are allowed to restore (Comms contacts, Admin, System Admin).
