...
| [src/components/ui/twelve-hour-time-field.tsx](../src/components/ui/twelve-hour-time-field.tsx)             | Optional `readOnly`: static 12h display; popover opens on focus (schedule row). All-day uses `Switch` in `ActivityScheduleSection`.                                             |
