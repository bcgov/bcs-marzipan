    ...
    fetchActivities()
      .then((activities) => {
        ...
        activities.forEach((activity) => {
          // ownerId is a string in the API response, convert to number
          if (activity.ownerId) {
            const ownerIdNum = parseInt(activity.ownerId.toString(), 10);
            if (!isNaN(ownerIdNum)) {
              uniqueLeadIds.add(ownerIdNum);
            }
          }
          // eventPlannerId is a string in the API response, convert to number
          if (activity.eventPlannerId) {
            const plannerIdNum = parseInt(
              activity.eventPlannerId.toString(),
              10
            );
            if (!isNaN(plannerIdNum)) {
              uniqueLeadIds.add(plannerIdNum);
            }
          }
        });
