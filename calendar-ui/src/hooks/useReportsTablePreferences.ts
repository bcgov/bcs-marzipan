...
/**
 * Persists reports table sort and filters using URL search params and sessionStorage
 * (`reportsTablePreferences` key), independent of the Activity List.
 * URL is source of truth when params are present (shareable links); when the user lands on
 * /reports with no params, state is restored from sessionStorage.
 */
export function useReportsTablePreferences(canSeeDeleted: boolean): {
